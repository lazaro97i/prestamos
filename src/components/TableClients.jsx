        ...
        <tbody >
          {
            clientStore?.clients?.response?.filter(
              e => e.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(filter)).length !== 0
              ?
              clientStore?.clients?.response?.filter(
                e => e.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(filter)).map((client) => {
                  return (
              ...
              :
              <tr>
                <td><p className='text-slate-950 font-[500] text-xl col-span-3 py-10 border-b border-slate-500 items-center'>No se encontraron datos !</p></td>
              </tr>
          }
        </tbody>
      </table>
